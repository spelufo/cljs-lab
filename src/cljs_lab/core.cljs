...
(defn bigram-char-style [a b]
  (let [f (rank-freq (get bigrams (str a b) 0))]
    {:border-right
      (if (> f 0)
        (str "3px solid rgba(21, 204, 18," f ")")
        (str "3px solid rgba(204, 21, 18," (- f) ")"))}))

; (defn bigram-col-style [ca cb] {})
(defn bigram-col-style [ca cb]
  (let [a (/ (reduce + (map #(get bigrams (str %1 %2) 0) ca cb)) (count ca))]
    {:border-right
      (if (> a 0.5)
        (str "5px solid rgba(21, 204, 18," (* 0.5 a) ")")
        (str "5px solid rgba(204, 21, 18," (- 0.5 a) ")"))}))
...
(defn column [props col next-col]
  [:div.column (assoc props :style (merge (:style props) (bigram-col-style col next-col)))
    (for [[j char] (zipmap (range) col)]
      [:div.char {:key j :style (bigram-char-style char (get next-col j))}
        char])])

(defn next-col [cols i]
  (let [ni (mod (inc i) (count cols))
        ncol (cols ni)]
    (if (= 0 ni)
      (str (subs ncol 1) (get ncol 0 ""))
      ncol)))
  ...
  (let [state (r/atom {:sel #{} :cols (transpose cipher)})]
    (fn []
      [:div.transposition-solver
        (let [sel (:sel @state)
              cols (:cols @state)]
          (for [[i col] (zipmap (range) cols)]
            [column {:key i
                     :tab-index (inc i)
                     :on-key-down (col-key-handler i state)
                     :on-click (select-cols i state)
                     :style (and (sel i) {:background-color "rgba(163,199,252,0.50)"})}
             col (next-col cols i)]))])))
